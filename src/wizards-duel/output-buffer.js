...
class OutputBuffer {
  ...
  flush() {
    ...
    for (let message of this.messages) {
      if (message.type === MESSAGE_SEND) sends.push(message.content);
      else replies.push(message.content);
    }
    sends.reverse();
    // console.log(sends);
    // console.log(this.messages);
    if (sends.length) this.response.send(...sends);

    if (replies.length) this.response.reply(...replies);

    this.messages = [];
  }

  send(output) {
    if (this.runningMessage) this.endMessage();
  ...
  endMessage() {
    if (this.runningMessage) {
      if (this.runningMessage.content) this.messages.push(this.runningMessage);
   ...
   */
  append(output) {
    if (!this.runningMessage) this.startMessage(MESSAGE_SEND);
  ...
  startReply() {
    this.startMessage(MESSAGE_REPLY);
  }
  endReply() {
    this.endMessage();
  }
