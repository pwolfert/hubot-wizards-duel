...
var effectConfigs = {
    ...
    synergies: [
      {
        ...
        modifiers: [["turnPain", "+=", 10, "is in slight pain from cold"]]
      },
      {
        effects: {
          and: ["fire", "tar"]
        },
        modifiers: [["turnPain", "+=", 10, "is in pain from burning"]]
      },
      {
        effects: {
          each: ["styrofoam", "hay"]
  ...
  blizzard: {},
  lightning: {
    modifiers: [["turnPain", "+=", 20, "is struck by lightning"]],
    synergies: [
      {
        effects: {
  ...
  "frog-vomitting": {
    ...
    determiner: Effect.POSSESSIVE_DETERMINER,
    isIllness: true,
    modifiers: [["turnSpellcasting", "-=", 20, "has difficulty speaking"]]
  },
  stench: {
    noun: "stench",
    determiner: Effect.POSSESSIVE_DETERMINER,
    counteracts: ["fragrance"],
    modifiers: [["turnSpellcasting", "-=", 10, "has difficulty concentrating"]]
  },
  fragrance: {
    noun: "fragrance",
    determiner: Effect.POSSESSIVE_DETERMINER,
  ...
  "merlins-beard": {
    ...
    determiner: "Merlin's",
    isFlammable: true,
    modifiers: [["turnSpellcasting", "+=", 20, "receives arcane wisdom"]]
  },
  "bowel-slickery": {
    noun: "bowel slickery",
    determiner: Effect.POSSESSIVE_DETERMINER,
    isIllness: true,
    modifiers: [["turnEvasion", "+=", 5, "has lubricated legs"]]
  },
  "bowel-stench": {
    noun: "bowel stench",
    determiner: Effect.POSSESSIVE_DETERMINER,
  ...
  "small-feet": {
    ...
    determiner: Effect.POSSESSIVE_DETERMINER,
    negates: ["large feet"],
    modifiers: [["turnEvasion", "-=", 10, "moves with greater difficulty"]]
  },
  "tiny-feet": {
    noun: "tiny feet",
    determiner: Effect.POSSESSIVE_DETERMINER,
    negates: ["large feet"],
    removes: ["small-feet"],
    modifiers: [["turnEvasion", "-=", 30, "cannot stand"]]
  },
  "swollen-tongue": {
    noun: "swollen tongue",
    determiner: Effect.POSSESSIVE_DETERMINER,
    modifiers: [["turnSpellcasting", "-=", 15, "has difficulty speaking"]]
  },
  "swollen-eyes": {
    noun: "swollen eyes",
    determiner: Effect.POSSESSIVE_DETERMINER,
  ...
  "eagle-head": {
    noun: "eagle head",
    determiner: Effect.POSSESSIVE_DETERMINER,
    modifiers: [["turnAccuracy", "+=", 10, "has eagle vision"]],
    synergies: [
      {
        effects: {
  ...
  "mer-tail": {
    ...
    modifiers: [["turnEvasion", "-=", 20, "cannot move effectively on land"]],
    synergies: [
      {
        effects: {
          or: ["flood-arena"]
        },
        modifiers: [["turnEvasion", "+=", 20, "is very agile in the water"]],
  ...
  clarity: {
    ...
    modifiers: [["turnSpellcasting", "+=", 10, "can think more clearly"]]
  },
  intoxication: {
    // Spoonerisms
    determiner: Effect.POSSESSIVE_DETERMINER,
      ...
      if (words.length > 1 && Math.random() < 0.6) {
        ...
        if (newIncantation !== spell.incantation) {
          manager.output.append(
            `@${player.state.name} tries to say _${spell.incantation}_ but drunkenly says _${newIncantation}_ instead.`
          );
          return false;
        }
      }
  ...
  "phonemic-confusion": {
    ...
    beforeCast: function(manager, player, spell, onSelf) {
      ...
      if (spell.incantation !== bsReplaced) {
        manager.output.append(
          `@${player.state.name} tries to say _${spell.incantation}_ but instead says _${bsReplaced}_.`
        );
        return false;
      }
    }
  ...
  fear: {
    determiner: Effect.POSSESSIVE_DETERMINER,
    modifiers: [["turnPain", "+=", 10, "is in mental anguish"]]
  },
  "inverted-vision": {
  ...
  "friendly-bard": {
    noun: "friendly bard",
    modifiers: [
      ["turnPain", "-=", 10, "can bear the pain of this world a little better"]
    ]
  },
  "unfriendly-bard": {
  ...
  "magic-shield-20": {
    ...
    modifiers: [
      ["turnShield", "+=", 20, "is provided with a good magical shield"]
    ],
  ...
  "magic-shield-30": {
    ...
    modifiers: [
      ["turnShield", "+=", 30, "is provided with a greater magical shield"]
    ],
  ...
  spectral: {
    beforeHit: function(manager, player, spell, onSelf) {
      manager.output.append(
        `The ${spell.projectileDescription} passes straight through @${player.state.name} with no effect`
      );
      return false;
    }
  },
...
var combinations = [
  [
    ["hemp-ropes", "peanut-butter", "mice"],
    ["mice"],
    "The mice, eating the peanut butter, chew through the ropes."
  ],
  [["peanut-butter", "mice"], ["mice"], "The mice eat the peanut butter."],
  [
    ["hemp-ropes", "peanut-butter", "rats"],
    ["rats"],
    "The rats, eating the peanut butter, chew through the ropes."
  ],
  ...
  [
    ["arena-cold", "arena-rain"],
    ["arena-hail"],
    "The rain freezes and turns to hail."
  ]
];
...
var Effects = {
  ...
  extractGlobalEffects(effectNames) {
    ...
    for (let effectName of effectNames) {
      if (Effects.get(effectName).global) globalEffects.push(effectName);
      else localEffects.push(effectName);
    }
  ...
  addEffect(currentEffectNames, effectName, output, playerName) {
    ...
    if (currentEffectNames.includes(effectName)) return currentEffectNames;
    ...
    for (let name of currentEffectNames) {
      ...
      if (currentEffect.alwaysRemoves(effectName)) {
        if (output) {
          ...
          output.append(
            `${Determiner} ${currentEffect.noun} ${currentEffect.removalVerb} ${effect.getDeterminer(playerName)} ${effect.noun}. `
          );
        }
        ...
        if (output) {
          ...
          let Determiner = d.charAt(0).toUpperCase() + d.slice(1);
          output.append(
            `${Determiner} ${currentEffect.noun} ${currentEffect.repellingVerb} ${effect.getDeterminer(playerName)} ${effect.noun}. `
          );
        }
        return currentEffectNames;
      }
    ...
    do {
      ({ remainingEffects, resultantEffects } = this.getCombinationResults(
        allEffectNames,
        output,
        playerName
      ));
    ...
    for (let newEffectName of newEffectNames) {
      ...
      if (negated.length) {
        ...
        if (output)
          output.append(
            `${Determiner} ${newEffect.noun} ${newEffect.negatingVerb} the ${oxfordJoin(negated)}. `
          );
      }
      ...
      if (output) {
        if (removed.length)
          output.append(
            `${Determiner} ${newEffect.noun} ${newEffect.removalVerb} the ${oxfordJoin(this.getNouns(removed))}. `
          );

        var counteracted = this.getCounteractedEffects(
          allEffectNames,
          newEffectName
        );
        if (counteracted.length)
          output.append(
            `${Determiner} ${newEffect.noun} counteracts the ${oxfordJoin(this.getNouns(counteracted))}. `
          );
      }
    }
  ...
  removeEffect(currentEffectNames, effectName, output, playerName) {
    if (output)
      output.append(
        `@${playerName}'s ${Effects.get(effectName).noun} has been removed. `
      );
      ...
      if (intersection.length === combination[0].length) {
        ...
        if (output) {
          if (combination[2]) {
            ...
            output.append(
              `The ${oxfordJoin(ingredientNouns)} combined, resulting in ${oxfordJoin(resultNouns)}. `
            );
          }
        }
      }
