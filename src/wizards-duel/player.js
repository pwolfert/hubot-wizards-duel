...
class Player {
	...
	get modifiedState() {
		if (!this._modifiedState)
			this._modifiedState = Player.getAffectedPlayerState(this.state);
	...
	attemptSpellCast(spell, onSelf) {
		...
		var modifiedState = this.modifiedState;
		...
		if (attemptCast) {
			...
			if (succeeded) {
				if (onSelf || this.spellHitTarget(spell)) {
					spell.cast(this, response, this.modifiedState, onSelf);
	...
	spellHitTarget(spell) {
		var opponent = new Player(this.state.opponent);
		var opponentState = opponent.modifiedState;
		var accuracy = Math.min(Math.max(this.modifiedState.turnAccuracy, MIN_ACCURACY), MAX_ACCURACY);
	...
	spellSucceeded(spell) {
		return (Math.random() >= this.modifiedState.turnSpellcasting);
	}
	...
	static getAffectedPlayerState(response, playerState, isDefense) {
