...
class Player {
  ...
  attemptSpellCast(spell, onSelf) {
    ...
    for (let effectName of affectedState.effects) {
      if (
        Effects.get(effectName).beforeCast(
          this.manager,
          this,
          spell,
          onSelf
  ...
  spellHitTarget(spell) {
    ...
    var accuracy = clamp(playerState.turnAccuracy, MIN_ACCURACY, MAX_ACCURACY) -
      clamp(playerState.turnPain, MIN_PAIN, MAX_PAIN);
    var evasion = clamp(opponentState.turnEvasion, MIN_EVASION, MAX_EVASION) -
      clamp(opponentState.turnPain, MIN_PAIN, MAX_PAIN);
    ...
    if (chanceToHit <= 0)
      ...
      for (let effectName of activeEffects) {
        if (
          Effects.get(effectName).beforeHit(
            this.manager,
            opponent,
            spell,
            false
  ...
  addEffect(effectName) {
    this.state.effects = Effects.addEffect(
      this.state.effects,
      effectName,
      this.output,
      this.state.name
  ...
  removeEffect(effectName) {
    this.state.effects = Effects.removeEffect(
      this.state.effects,
      effectName,
      this.output,
      this.state.name
  ...
  getAffectedState(isDefense) {
    if (isDefense) {
      if (!this._defenseAffectedState)
        this._defenseAffectedState = Player.getAffectedPlayerState(
          this.manager,
          this.state,
          isDefense
        );
      ...
      if (!this._affectedState)
        this._affectedState = Player.getAffectedPlayerState(
          this.manager,
          this.state,
          isDefense
        );
  ...
  static getActiveEffects(playerState) {
    ...
    for (let i = 0; i < effects.length; i++) {
      ...
      if (!counteracted) activeEffects.push(effects[i]);
    }
  ...
  getStatus() {
    ...
    if (state.turnPain > 0)
      lines.push(
        `You are in ${Language.getSeverityAdjective(state.turnPain)} pain`
      );
    else
    ...
    if (state.turnShield > 0)
      lines.push(
        `and you have a ${Language.getAdvantageAdjective(state.turnShield)} shield.`
      );
    else
